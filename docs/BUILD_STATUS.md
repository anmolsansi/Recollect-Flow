...
- OPE-223 multimodal extraction for PDFs (via `pdfjs-dist`) and images (via OpenRouter/Gemini), decoupled into `extraction_records` without mutating the canonical source. Fully implemented with capabilities metadata, error boundaries, and integration into the enrichment pipeline.
