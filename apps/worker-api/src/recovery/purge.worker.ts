...
async function runWorkflow(
  ...
  if (!(await repository.markWorkflowProcessing(workflow.id, now()))) return;
...
export async function processPurgeWorkflow(
  env: Env,
  workflowId: string,
  options: PurgeWorkerOptions = {},
): Promise<void> {
...
export async function processPurgeWorkflows(
  ...
  for (const workflow of workflows) await runWorkflow(env, workflow, options);
}
