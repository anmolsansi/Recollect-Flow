...
export function itemRoutes() {
  ...
  router.get('/items/:id', requireAdminToken, async (context) => {
    ...
    const [attachments, extractions, captureEvents, providerUsage, feedback, audit, overrides] =
      await Promise.all([
        db.prepare(
    ...
    return context.json(
      itemDetailResponseSchema.parse({
        data: {
          item: { ...item, topics: parseTopics(item.topics_json as string | null) },
  ...
  router.post('/items/:id/status', requireAdminToken, async (context) => {
    ...
    const result = await db.batch([
      ...
      ).bind(parsed.data.lifecycle_status, now, itemId, parsed.data.edit_version),
      db.prepare(
        `INSERT INTO audit_events
           (id, item_id, event_type, actor_type, details_json, created_at)
  ...
  router.post('/items/:id/duplicate', requireAdminToken, async (context) => {
    ...
    if (parsed.data.duplicate_of === itemId) {
      throw new AppError(409, 'INVALID_DUPLICATE_TARGET', 'An item cannot duplicate itself');
    }
    const target = await db
      .prepare('SELECT id FROM items WHERE id = ?1 AND deleted_at IS NULL')
      .bind(parsed.data.duplicate_of)
      .first();
    if (!target) throw new AppError(404, 'NOT_FOUND', 'Duplicate target not found');
  ...
  router.post('/items/:id/overrides/reset', requireAdminToken, async (context) => {
    const itemId = context.req.param('id');
