...
async function clearRecovery(): Promise<void> {
  ...
  if (receiptObjects.objects.length) {
    await env.ATTACHMENTS.delete(receiptObjects.objects.map((object) => object.key));
  }
}
  ...
  it('recreates canonical fields, history and FTS usability in a clean database', async () => {
    ...
    expect(
      await env.DB.prepare(
        'SELECT id FROM capture_events WHERE item_id = ?1',
      )
        .bind(ITEM_ID)
        .first(),
    ).not.toBeNull();
    ...
    expect(
      await env.DB.prepare(
        'SELECT id FROM sync_attempts WHERE item_id = ?1',
      )
        .bind(ITEM_ID)
        .first(),
    ).not.toBeNull();
